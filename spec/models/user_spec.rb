...
RSpec.describe User, type: :model do
  context "prepaid course" do
    let(:course) { FactoryBot.create(:course, payment_option: 'Prepaid') }
    let(:chamber) { FactoryBot.create(:chamber, course: course) }

    xit "does sends activation email to Members" do
      member_auth_code = chamber.authorization_codes.find_by_chamber_role_type("Member")

      user = FactoryBot.build(:user)
      chamber_role = member_auth_code.create_chamber_role( user )

      expect(user).to receive(:send_activation_code)
      user.register!
    end

    xit "does sends activation email to Administrator" do
      admin_auth_code = chamber.authorization_codes.find_by_chamber_role_type("Administrator")

      user = FactoryBot.build(:user)
      chamber_role = admin_auth_code.create_chamber_role( user )
  ...
  context "foliodirect course" do
    let(:course) { FactoryBot.create(:course, payment_option: 'FolioDirect') }
    let(:chamber) { FactoryBot.create(:chamber, course: course) }

    xit "does not send activation email to Members" do
      member_auth_code = chamber.authorization_codes.find_by_chamber_role_type("Member")

      user = FactoryBot.build(:user)
      chamber_role = member_auth_code.create_chamber_role( user )

      expect(user).to_not receive(:send_activation_code)
      user.register!
    end
